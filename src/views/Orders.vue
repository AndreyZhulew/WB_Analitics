<template>
  <div class="orders-page">
    <div class="page-header">
      <h1>Заказы</h1>
      <el-tag type="primary">Аналитика</el-tag>
    </div>
    ...
    <div class="stats-cards" v-if="!loading && statsData.total > 0">
      <el-row :gutter="20">
        ...
        <el-col :span="8">
          <el-card class="stat-card">
            <div class="stat-content">
              <div class="stat-value">{{ formatCurrency(statsData.averagePrice) }}</div>
...
const { 
  ...
  setPerPage,
  refresh
} = useApiData(apiService.getOrders)
...
const chartData = computed(() => {
  ...
  data.value.forEach(item => {
    if (item.date) {
      ...
      grouped[item.date] += item.quantity || 0
    }
  })
...
const statsData = computed(() => {
  if (data.value.length === 0) return { count: 0, totalQuantity: 0, averagePrice: 0 }
  
  const count = data.value.length
  const totalQuantity = data.value.reduce((sum, item) => sum + (item.quantity || 0), 0)
  const totalPrice = data.value.reduce((sum, item) => sum + (item.totalprice || 0), 0)
